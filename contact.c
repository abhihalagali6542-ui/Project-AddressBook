...
int name_validtion(char *name,AddressBook *addressBook)
{
    while(1)
    {
          for(int i=0;i<addressBook->contactCount;i++)
...
int phone_validtion(char *phone,AddressBook *addressBook)
{
    ...
    if(phone_len<10 && phone_len>10 || phone[0]<'6')
...
int email_validtion(char *email,AddressBook *addressBook)
{
...
void createContact(AddressBook *addressBook)
{
	/* Define the logic to create a Contacts */
        ...
        while(!name_validtion(addressBook->contacts[addressBook->contactCount].name,addressBook))
        {
                scanf("%s",addressBook->contacts[addressBook->contactCount].name);
        }
        ...
        while(!phone_validtion(addressBook->contacts[addressBook->contactCount].phone,addressBook))
        {
            scanf("%s",addressBook->contacts[addressBook->contactCount].phone);
        }       
        ...
        while(!email_validtion(addressBook->contacts[addressBook->contactCount].email,addressBook))
        {
            scanf("%s",addressBook->contacts[addressBook->contactCount].email);
        }        
...
void search_desplay_name(AddressBook *addressBook, char *whatever)
{
    printf("Sl.no\tName\t\tPhone Number\tEmail ID\n");
     for(int i=0;i<addressBook->contactCount;i++)
     {
         if(strstr(addressBook->contacts[i].name, whatever)!=NULL)
         {
            printf("  %d\t%s\t\t%s\t%s\n",i+1,addressBook->contacts[i].name,
                addressBook->contacts[i].phone,addressBook->contacts[i].email);
         }
